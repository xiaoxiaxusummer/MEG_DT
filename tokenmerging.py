...
def compute_merge(x: torch.Tensor, tome_info: Dict[str, Any]) -> Tuple[Callable, ...]:
    ...
    downsample = int(math.ceil(math.sqrt(original_tokens // x.shape[1]))) # 目前已下采样的级别
    ...
    if downsample <= args["max_downsample"]:
        ...
        r = int(x.shape[1] * args["ratio"]) # 删除的数目
...
def apply_patch(
    ...
    transformer_layers = 0 # 记录需要merging的Transformer层数
    for _, module in diffusion_model.named_modules():
        # If for some reason this has a different name, create an issue and I'll fix it
        if isinstance_str(module, "BasicTransformerBlock"):
            make_tome_block_fn = make_diffusers_tome_block if is_diffusers else make_tome_block
            module.__class__ = make_tome_block_fn(module.__class__) # 将BasicTransformerBlock换成ToMeBlock
            module._tome_info = diffusion_model._tome_info
            module._tome_info["args"]["_id"] = transformer_layers # 标记当前处理的是第几个Transformer layer
