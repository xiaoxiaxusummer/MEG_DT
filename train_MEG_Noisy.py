...
from MEG_Net import SplitModel
...
parser = argparse.ArgumentParser(description='Training MEG feature decoders over noisy Gaussian channels')
...
parser.add_argument('--save_dir', type=str, default = project_dir + '/assets/MEG_noisy/')
...
test_img_path = project_dir + f'/result/train/generated_images/MEG_noisy/'
...
# ========================== load JSCC model ========================
MEG_model = SplitModel(args).to(device, args.dtype)
# For initialization, load a pretrained VAE decoder
vae = AutoencoderKL.from_pretrained("madebyollin/sdxl-vae-fp16-fix", torch_dtype=args.dtype).to(device, args.dtype)
MEG_model.post_quant_conv.load_state_dict(vae.post_quant_conv.state_dict())
decoder_state_dict = torch.load(os.path.join(project_dir,'assets/decoder_model/LAIONCOCO_ckpt_299_model.pth'))
MEG_model.decoder.load_state_dict(decoder_state_dict['decoder'])
...
def train(config, MEG_model, train_dataset, optimizer, lr_scheduler, end_epoch, SNR_dB=0):
    test_MSE =  torch.inf
    ...
    MEG_model, optimizer, train_dataset, lr_scheduler = accelerator.prepare(
        MEG_model, optimizer, train_dataset, lr_scheduler
    )

    global_step = 0
    MEG_model.train()
    MEG_model.requires_grad_(True)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for step_i in range(n_step):
            ...
            with accelerator.accumulate(MEG_model):
                img_pred, _ = MEG_model(latents, SNR_dB, is_train=True)
                loss = torch.nn.functional.mse_loss(img_pred,sdxl_image,reduction="mean")
                if torch.isnan(loss):
                    raise Exception("NaN value")
                accelerator.backward(loss)
                accelerator.clip_grad_norm_(MEG_model.parameters(), 1.0)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
        ...
        if accelerator.is_main_process:
            MSE_batch = test(epoch, MEG_model, SNR_dB)
            if (epoch + 1) % config.save_model_epochs == 0 or epoch == end_epoch - 1:
                torch.save({'Split_Model': copy.deepcopy(MEG_model.state_dict()), 'epoch':epoch, "config":config, "args": args}, 
                           args.save_dir + f"/LAIONCOCO_ckpt_{epoch}_model.pth")

        print('Test MSE:',MSE_batch)

        if epoch > 180:
            if (MSE_batch < test_MSE):
                test_MSE = MSE_batch
                print('Best ckpt -- MSE:',MSE_batch)
    torch.save({'Split_Model': copy.deepcopy(MEG_model.state_dict()), 'epoch':epoch, "config":config, "args": args}, args.save_dir + f"/LAIONCOCO_final_ckpt_model.pth")
    return MEG_model

def test(epoch, MEG_model, SNR_dB = 0):
    ...
    with torch.no_grad():
        ...
        for step_i in range(n_step):
            ...
            img_pred, noise = MEG_model(latents, SNR_dB)
    ...
    if args.test == 0:
        ...
        for e_idx, end_epoch in enumerate(config.end_epochs):
            if start_epoch < end_epoch:
                optimizer = torch.optim.AdamW(MEG_model.parameters(), lr=config.learning_rate[e_idx])
                ...
                MEG_model = train(config, MEG_model, train_dataset, optimizer, lr_scheduler, end_epoch, train_SNR_dB) 
