...
"""
We performs token merging for both transmitted feature compression and self-attention computation acceleration in this work. 
"""
...
def compute_merge(x: torch.Tensor, tome_info: Dict[str, Any]) -> Tuple[Callable, ...]:
    ...
    downsample = int(math.ceil(math.sqrt(original_tokens // x.shape[1]))) # Current downsampling level
    ...
    if downsample <= args["max_downsample"]:
        ...
        r = int(x.shape[1] * args["ratio"]) # Number of dropped tokens
...
""" Used for token merging during computing"""
...
def apply_patch(
    ...
    transformer_layers = 0 # Initialize the number of Transformer layers to be handled
    for _, module in diffusion_model.named_modules():
        if isinstance_str(module, "BasicTransformerBlock"):
            make_tome_block_fn = make_diffusers_tome_block if is_diffusers else make_tome_block
            module.__class__ = make_tome_block_fn(module.__class__) # Replace BasicTransformerBlock into ToMeBlock
            module._tome_info = diffusion_model._tome_info
            module._tome_info["args"]["_id"] = transformer_layers # The Transformer layer to be handled currently
...
def bipartite_soft_matching_random2d(metric: torch.Tensor,
    ...
    """
    B, N, _ = metric.shape # number of batches and features
