...
from MEG_Net import JSCCMergingNet, SplitModel
...
parser = argparse.ArgumentParser(description='Training E2E-MEG model with Transmitted Feature Merging and End-to-End Decoding')
...
parser.add_argument('--save_dir', type=str, default = project_dir + '/assets/E2E_MEG/')
...
test_img_path = project_dir + f'/result/train/generated_images/E2E_MEG/'
...
# ==========================  Load MEG-Feature Merging model  ========================
E2E_MEG_model = JSCCMergingNet(args, merging_ratio=0.5, use_conv_encoder=True, conv_config=[1,0]).to(device, args.dtype)
# Start with pretrained split MEG model to finetune
MEG_base_model = SplitModel(args).to(device, args.dtype)
MEG_base_model_data = torch.load(os.path.join(project_dir,'assets/MEG_noisy/LAIONCOCO_ckpt_649_model.pth'))
MEG_base_model.load_state_dict(MEG_base_model_data["Split_Model"])
E2E_MEG_model.post_quant_conv.load_state_dict(MEG_base_model.post_quant_conv.state_dict())
E2E_MEG_model.decoder.load_state_dict(MEG_base_model.decoder.state_dict())
del MEG_base_model, MEG_base_model_data
...
if not args.resume_vae:
    ...
    E2E_MEG_model.load_state_dict(data['JSCC_model'])
    del data

torch.cuda.empty_cache()


def train(config, E2E_MEG_model, train_dataset, optimizer, lr_scheduler, end_epoch, SNR_dB=0):
    test_MSE =  torch.inf
    ...
    E2E_MEG_model, optimizer, train_dataset, lr_scheduler = accelerator.prepare(
        E2E_MEG_model, optimizer, train_dataset, lr_scheduler
    )

    global_step = 0
    E2E_MEG_model.train()
    E2E_MEG_model.requires_grad_(True)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for step_i in range(n_step):
            ...
            with accelerator.accumulate(E2E_MEG_model):
                img_pred, _ = E2E_MEG_model(latents, SNR_dB, is_train=True)
                loss = torch.nn.functional.mse_loss(img_pred,sdxl_image,reduction="mean")
                if torch.isnan(loss):
                    raise Exception("NaN value")
                accelerator.backward(loss)
                accelerator.clip_grad_norm_(E2E_MEG_model.parameters(), 1.0)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
        ...
        if accelerator.is_main_process:
            MSE_batch = test(epoch, E2E_MEG_model, SNR_dB)
            if (epoch + 1) % config.save_model_epochs == 0 or epoch == end_epoch - 1:
                torch.save({'JSCC_model': copy.deepcopy(E2E_MEG_model.state_dict()), 'epoch':epoch, "config":config, "args": args}, 
                           args.save_dir + f"/LAIONCOCO_ckpt_{epoch}_model.pth")

        print('Test MSE:', MSE_batch, 'info:', E2E_MEG_model.get_info_dim())

        if epoch > 180:
            if (MSE_batch < test_MSE):
                test_MSE = MSE_batch
                print('Best ckpt -- MSE:',MSE_batch)
    torch.save({'JSCC_model': copy.deepcopy(E2E_MEG_model.state_dict()), 'epoch':epoch, "config":config, "args": args}, args.save_dir + f"/LAIONCOCO_final_ckpt_model.pth")
    return E2E_MEG_model

def test(epoch, E2E_MEG_model, SNR_dB = 0):
    ...
    with torch.no_grad():
        ...
        for step_i in range(n_step):
            ...
            img_pred, noise = E2E_MEG_model(latents, SNR_dB)
...
if __name__=='__main__':
    ...
    for e_idx, end_epoch in enumerate(config.end_epochs):
        if start_epoch < end_epoch:
            optimizer = torch.optim.AdamW(E2E_MEG_model.parameters(), lr=config.learning_rate[e_idx])
            ...
            E2E_MEG_model = train(config, E2E_MEG_model, train_dataset, optimizer, lr_scheduler, end_epoch, train_SNR_dB) 
